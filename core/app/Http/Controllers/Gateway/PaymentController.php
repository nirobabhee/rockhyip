...
class PaymentController extends Controller
{
    ...
    public function deposit()
    {
        ...
        //         return redirect()->route('user.plans')->withNotify($notify);
        //     }
    ...
    public function depositInsert(Request $request)
    {
        ...
        $session = session('checkout');
        $amount = @$session['amount'] ?? $request->amount;
        ...
        if ($plan_id) {
            ...
            if ($amount != $plan->maximum_investment || $amount < $plan->minimum_investment && $amount > $plan->maximum_investment) {
                $notify[] = ['error', 'Invalid plan request'];
                return redirect()->route('all.plan')->withNotify($notify);
            }
        }
        //End Plan Area
        $user = auth()->user();
    ...
    public static function userDataUpdate($trx)
    {
        ...
        if ($data->status == 0) {
            ...
            $user->save();
            //Start-Multi-referral-deposit
            $referral = $user;
            if ($referral->ref_by) {
                referralLevel($referral, 1, $data, $user, $data->trx);
            }
            //End-Multi-referral-deposit
            ...
            if ($data->plan_id) {
                ...
                if ($plan) {
                    ...
                    $investment->interest  = $plan->interest_status == 0 ? $plan->interest : $data->amount * $plan->interest / 100;
                    ...
                    $investment->details = $plan->name . ' Plan invested by ' . $general->cur_sym . showAmount($investment->amount);
                    $investment->details = 'Successfully ' . $plan->name . 'plan invested.';
                    $investment->save();
                    //Transaction
                    $transaction = new Transaction();
                    $transaction->user_id = $data->user_id;
                    ...
                    $transaction->charge = $data->charge;
                    $transaction->trx_type = '-';
                    $transaction->details = $plan->name . ' Plan invested by ' . $general->cur_sym . showAmount($investment->amount) . ' via ' . $data->gatewayCurrency()->name;
                    $transaction->trx = $data->trx;
                    $transaction->save();
                    //Plan Admin Notification
                    $adminNotification = new AdminNotification();
                    $adminNotification->user_id = $user->id;
                    ...
                    $adminNotification->save();
                    session()->put('redirect_plan', $investment->plan_id);
                }
            } else {

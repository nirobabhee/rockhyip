...
function referralLevel($referral, $type, $data, $referralForm, $trx)
{
    $level = ReferralLevel::where('type', $type)->orderBy('id', 'asc')->get();
    for ($x = 0; $x < count($level); $x++) {
        if (!$referral->ref_by) {
            break;
        }
        $referralPercent = $level[$x]->percent;
        $referralTo = User::find($referral->ref_by);
        $refCommission = $data->amount * $referralPercent / 100;
        $referralTo->interest_wallet += $refCommission;
        $referralTo->save();
        $transaction = new Transaction();
        $transaction->user_id = $referralTo->id;
        $transaction->amount = $refCommission;
        $transaction->charge = 0;
        $transaction->post_balance = $referralTo->interest_wallet;
        $transaction->trx_type = '+';
        $transaction->trx = $trx;
        $transaction->details =  'Reference Comission by ' . $referralForm->username;
        $transaction->save();
        $log = new CommissionLog();
        $log->from_id = $referralForm->id;
        $log->to_id = $referralTo->id;
        $log->type =  $level->type = 2 ? 2 : 1;
        $log->level =  $level[$x]->level;
        $log->percent =  $referralPercent;
        $log->amount =  $refCommission;
        $log->trx =  $trx;
        $log->save();
        $referral = $referralTo; //(referral - iterate)find next ref//
    }
}
